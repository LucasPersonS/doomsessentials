...
public final class StorageEnvValidator {
    ...
    public static void validateServer() {
        ...
        if (lastRunMs != 0 && (now - lastRunMs) < 30_000)
            return;
        lastRunMs = now;
        ...
        int viewMax = 64;
        EssentialsMod.LOGGER.info("StorageEnv: dedicated=" + dedicated + ", production=" + FMLEnvironment.production
                + ", dist=" + net.minecraftforge.fml.loading.FMLEnvironment.dist.name() + ", motd=" + motd
                + ", viewMax=" + viewMax + ", debug="
                + org.lupz.doomsdayessentials.guild.GuildConfig.STORAGE_DEBUG_ENABLED.get());
        try {
            if (viewMax != 64)
                EssentialsMod.LOGGER.warn("StorageEnv: view max stack size unexpected=" + viewMax);
